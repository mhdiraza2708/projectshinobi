...
- **No motion-capture clips committed.** Mixamo clips plug in (see
  ...
  is procedural IK.
- **Sound effects are synthesised, and there's no music yet.** Every sound is
  generated by `art/audio/make_sfx.py` (original, no licensing). They're
  game-jam quality, not studio foley. Drop recorded sounds with the same file
  names into `game/assets/audio/sfx/` to replace any of them.
